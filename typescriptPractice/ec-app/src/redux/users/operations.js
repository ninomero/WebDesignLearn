...
export const signUp = (username, email, password, confirmPassword) => {
    return async () => {
        ...
        [email, ] = useContext(MyContext),
        [password, ] = useContext(MyContext),
        [confirmPassword, ] = useContext(MyContext);
        ...
        // firebaseの設定で作成したauthを使用
        // createUserWithEmailAndPassword = アカウント作成コンポーネント
        return auth.createUserWithEmailAndPassword(email, password)
            .then(result => {
                // dispatch(showLoadingAction("Sign up..."))
                const user = result.user;
                // 作成したユーザの情報を使用して、ユーザ用のデータを作成してfirebaseに登録する
                if (user) {
                    const uid = user.uid;
